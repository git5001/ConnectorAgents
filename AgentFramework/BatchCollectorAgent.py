...
class BatchCollectorAgent(ConnectedAgent):
    ...
    # Lifecycle
    # --------------------------------------------------------------------
    def __init__(self, config: BatchCollectorAgentConfig, uuid: str = "default") -> None:
    ...
    # Core functionality
    # --------------------------------------------------------------------
    def process(self, params: BaseIOSchema, parents: List[str]) -> Optional[ListModel]:
        ...
        if len(self._state.buffer) >= self.config.amount:
            ...
            processed: List[BaseIOSchema] = [self.run(msg) for msg in batch]
